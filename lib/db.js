...
async function ensureIndexes(db) {
  ...
  await db.collection("walletTransactions").createIndex({ txHash: 1 }, { unique: true, partialFilterExpression: { txHash: { $type: "string" } } });
  ...
  await db.collection("withdrawalRequests").createIndex({ txHash: 1 }, { unique: true, partialFilterExpression: { txHash: { $type: "string" } } });
...
export async function createWalletTransaction(transaction, session = null) {
  ...
  const tx = {
    ...
    depositMethod: typeof transaction.depositMethod === "string" ? transaction.depositMethod.trim().toLowerCase() : null,
